...
## 7. +8..+13 = 6 faction 지배국 충성도 (2026-05-23 agent 통계)

130 ports 의 record 전수 분석 (평균물가 % 위치 탐색 과정의 부수 발견):

### 7-1. 패턴

100개 표준 항구 (idx 0..99) 의 +8..+13 (6 byte) 분포:
- 6 byte 중 **최대 1 byte 만 nonzero** (약 65 ports). 약 35 ports 는 전부 0 (독립/중립).
- nonzero 값 범위: 0x14..0x64 (20..100)
- nonzero 위치 = 항구 국적과 1:1 일치

### 7-2. faction ↔ byte 위치 매핑

| offset | faction | anchor |
|---|---|---|
| +8  | **포르투갈** | 리스본(0)=100, 마데이라(57)=100, 세우타(26)=100 |
| +9  | **스페인** | 세빌리아(1)=100 |
| +10 | ? (지중해/이탈리아 후보) | 베네치아/제노바 추정 — 검증 미완 |
| +11 | **잉글랜드** | 런던(29)=100 |
| +12 | ? | 미확정 |
| +13 | **네덜란드** | 암스테르담(33)=100 |

값 = 충성도/지배도 % (0..100).

### 7-3. 신규 편집 기능 후보

- 새 sub-tab "항구 지배국" 또는 항구 탭에 컬럼 추가 가능
- API: `horiedit_py/data/port.py` 에 `load_port_faction(state, idx)` / `save_port_faction(...)` 추가
- record offset: +8, size 6
- 안전 장치: 사용자가 여러 byte 를 동시에 nonzero 로 설정하면 게임 혼란 가능성 — radio button 형식으로 single-faction 강제 권장
- 미확정 (검증 필요): +10, +12 의 정확한 faction / 충성도 변경의 게임 내 시각적·메커닉 효과 (지도 색? 세금? NPC 함대?)

### 7-4. 다른 byte 영역 (보조 발견)

| offset | 의미 추정 |
|---|---|
| +24..25 | 방위/병력 (u16 LE, 매우 광범위 값 + 0xFF00 placeholder) |
| +26, +27 | 다른 u8 stat (6..60 범위) |
| +28 | 지역/대륙 ID (0..7 sequential) |
| +29..32 | 이벤트 플래그 (대부분 0xFF placeholder) |
| +33, +34, +36 | zone 분류 (0..10) |
