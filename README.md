...
- **함선명·선박명·인물 성/이름의 마지막 한글이 깨진 문자(�)로 저장되는 버그 수정**
    ([#12](https://github.com/genishs/dh2-py-editor/issues/12)) —
    `encode_kr_fixed()` 가 고정 버퍼에 맞춰 자를 때 바이트 단위로 잘라
    2바이트 EUC-KR 문자 한가운데가 잘렸다. 절단을 **문자 단위**로 바꿔
    완전한 글자만 저장되도록 수정 (넘치는 마지막 글자는 통째로 잘린다).
    예: 함선명 14바이트 칸에 `바다의별호가나` 입력 시
    이전 `바다의별호가�` → 이후 `바다의별호가`.
...
- **충성도는 주인공 동료일 때만 표시** — 충성도(`none1`)는 주인공 동료에게만
    ...
    [충성도] 입력칸이 보이도록 변경. 그 외(정착민/NPC/모집 가능)에는 숨김.
    `person_tab._update_loyalty_visibility()` 가 `pos` 변경/인물 로드 시 토글.
- **인물 탭 레이아웃 압축** — 소속 직접 입력 안내(3줄→1줄)와 충성도 안내
    (2줄→1줄)를 줄이고, 충성도 행을 동료일 때만 노출해 기본 창 크기(980×700)
    에서 인물 탭 전체 내용이 보이도록 정리.
- (참고) 동료 항해사 **월 급료** 편집은 별도 이슈([#10](https://github.com/genishs/dh2-py-editor/issues/10))
    로 분리. 분석 결과 급료는 세이브의 인물 레코드(50 byte)에 없고 게임 본체
    파일(MAIN.EXE)의 항해사 카탈로그에 고정 저장된 것으로 확인 — 추가 역공학 필요.

### v0.4.16 (2026-05-30 — 새 기능 소개 '다시 보지 않기' ([#7](https://github.com/genishs/dh2-py-editor/issues/7)) + 함선 기본값 초기화 ([#8](https://github.com/genishs/dh2-py-editor/issues/8)))

- **[#7] 새 기능 소개 '시작 시 열기' 체크박스** — "새 기능 소개" 창 하단에
    `프로그램 시작 시 새 기능 소개 창 열기` 체크박스 추가. 자동으로 뜬 창과
    [도움말] ▸ [새 기능 소개] 로 연 창 **모두 동일한 체크박스**를 표시하며,
    체크 상태를 게임 폴더 옆 `.dh2editor_seen` 에 저장(`1`/`0`)한다. 체크를
    끄고 닫으면 다음 실행부터 자동으로 안 뜨고, 도움말에서 다시 켜면 시작 시
    다시 뜬다. (기존 '다시 보지 않기' 의미를 '시작 시 열기' 토글로 통일.)
    (`help_dialog._show_scrolling_text` 에 `checkbox_label`/`checkbox_initial`/
    `on_close` 추가, X 버튼 닫기도 콜백 처리.)
- **[#8] 함선 '기본값으로 초기화'** — [선박] ▸ [원래 함선 정보] 에
    `이 배를 기본값으로 초기화` 버튼 추가. 게임 본체 파일(MAIN.EXE)에 저장된
    그 함선의 원본 값(성능/적재량/선원수/내구도/가격/분류/이름)으로 한 번에
    복원한다. 기존 저장 로직을 재사용해 영웅 함대로의 클램프 전파도 함께
    적용. 게임 본체 파일이 없으면 안내 후 비활성. 실수 방지 확인 다이얼로그.
- **[#9] 인물 충성도 편집** — [인물] 탭의 [소속 / 정착 항구] 영역에
    `충성도` (0~255) 입력칸 추가. Person 의 `none1` (+43) 값을 직접 편집.
    동료 등록 시 자동으로 채워지는 값(0→100)보다 사용자 입력을 우선 적용하되,
    동료/모집 가능 전환의 자동 sync 와 충돌하지 않게 처리.

### v0.4.15 (2026-05-30 — UI 개선: 용어 순화 / 안내 배너 / 새 기능 소개 / 풋터 고정 ([#6](https://github.com/genishs/dh2-py-editor/issues/6)))

이슈 [#6](https://github.com/genishs/dh2-py-editor/issues/6) 의 UI 의견 3가지를 반영.

- **어려운 용어 순화** (#6-1): 사용자에게 보이는 GUI 문자열에서 컴퓨터/구현
    용어를 일반 유저용으로 정리. 예) `MAIN.EXE` → "게임 본체 파일(MAIN.EXE)",
    `KOUKAI2.DAT`/전체 경로 → "저장 파일(KOUKAI2.DAT)" / 파일명만, "슬롯" →
    "세이브 칸", "패치 적용" → "적용", `Ship4/Ship5 ROM` → "함선 기본 성능" /
    "함선 이름·모양", `byte`/`offset`/16진수/`pos`/`catalog` 등은 숨김. 함선·칸
    번호는 0-base → 1-base 로 통일. 백업 안내는 정확성을 위해 실제 파일명을
    괄호로 병기 유지. (`help_dialog`, `hull_cap_dialog`, `settings_tab`,
    `person_tab`, `ship_tab`, `port_tab`, `slot_select`, `app`)
- **기능 발견성 향상** (#6-2):
    - 상단 **회전형 안내 배너** (`_build_notice_banner`) — 발견성이 낮은 기능
        (최대 내구도/동료 등록/도움말 등)을 8초마다 순환 안내. 닫기(×) 가능.
        "최대 내구도" 팁은 클릭 시 바로 해당 창을 연다.
    - **새 기능 소개 모달** (`show_whats_new`) — 최초 실행 및 버전업 시 1회
        자동 표시. [도움말] ▸ [새 기능 소개] 에서 언제든 다시 볼 수 있음.
        표시 기록은 게임 폴더 옆 `.dh2editor_seen` 에 마지막 본 버전을 저장.
- **풋터(상태바) 항상 표시** (#6-3): `app.py::_build()` 에서 상태바를
    notebook(expand) 보다 **먼저** pack 하여 작은 창에서도 잘리지 않게 함.
    `minsize` 580 → 640 으로 상향.
- 상태바는 전체 경로 대신 파일명만 표시 ("저장 파일: KOUKAI2.DAT | 세이브 칸: 1").
